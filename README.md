...
Integration with WebView2: Interaction with the web interface of the DeepSeek language model is implemented through the implementation of JavaScript scripts. This emulates filling out a form and sending requests, and also allows you to change the AI provider in the future without rebuilding the expansion architecture.
...
Key components:
AiAssistantWindowControl is the main chat assistant window implemented in XAML.
ChangeApplier is an engine for applying changes that includes algorithms for searching code with whitespace normalization and calculating replacement positions.
FileDiff is a data model for storing the status of a modified file (original content, path, return flag).
EmbeddingService is a microservice for semantic code search based on the ElBruno library.

AI integration method: DOM manipulation via JavaScript in WebView2 control to interact with the DeepSeek web client.
